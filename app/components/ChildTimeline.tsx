...
function groupByDay(events: EventRow[]): [string, EventRow[]][] {
  ...
  // newest start first; on a tie, an event WITH an end time sorts ABOVE one
  // without (it extends past the shared start, so it's happening later)
  for (const items of map.values()) {
    items.sort((a, b) => {
      const d = +new Date(b.start_time) - +new Date(a.start_time);
      if (d) return d;
      return (b.end_time ? 1 : 0) - (a.end_time ? 1 : 0);
    });
  }
